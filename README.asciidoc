...
DPDK has serveral arguments that determine target, not just ISA (X86 or arm). Within DPDK arm support there are esentually a generaic arm64 server build target -   and a number of arm vendor specific targets. TRex build environment ONLY understands ISA target and not platform variances.

This port builds a generic arm64 server (PCIe connected PMD/NICs) that supports most PMD's (MLX can be enabled) and has been tested with severa arm64 server solutions and AF_PACKET PMD, Intel NIC PMD's.
